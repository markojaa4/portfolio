...
/*
	Bulls and Cows game
	Guess a sequence of four unique digits
	1 cow for each correct digit not in its place
	1 bull for each correct digit in correct position
*/
...
/*
	Ask for an integer to use for seed to generate the numbers.
...
try {
	...
	for (;;) {
		...
		srand(seed);
		cout << "Guess a sequence of four unique digits (0 - 9):\n";

		// generate the target sequence
		vector<int> target;
		for (unsigned int i = 1; i <= 4; ++i) target.push_back(rand() % 10);
		...
		while (guess != target) {
			...
			for (int val = 0; guess.size() < 4 && cin >> val;) {
				if (val < 0 || val > 9) error("invalid input", "; use single digit numbers only!");
				guess.push_back(val);
			}
